    ...
    <main className="inventory-page">
      ...
      <section className="inventory-catalog ss-card">
        <div className="inventory-toolbar">
          <label className="inventory-search">{icon("M21 21l-4.35-4.35M19 11a8 8 0 1 1-16 0 8 8 0 0 1 16 0z")}<input value={query} onChange={event => setQuery(event.target.value)} placeholder="Search name, SKU, or category..." aria-label="Search inventory" data-tooltip="Search product records by name, SKU, or category." /></label>
          <div className="inventory-filters" aria-label="Filter products">{(["all", "active", "low", "out"] as const).map(filter => <button type="button" key={filter} className={status === filter ? "active" : ""} onClick={() => setStatus(filter)} data-tooltip={"Filter products by " + (filter === "all" ? "all statuses" : filter === "low" ? "low stock" : filter === "out" ? "unavailable status" : "active status") + "."}>{filter === "all" ? "All" : filter === "active" ? "Active" : filter === "low" ? "Low stock" : "Unavailable"}</button>)}</div>
